 ...
 * @LastEditTime: 2021-12-21 15:03:09
  ...
  <div class="top-category">
     ...
     <!-- 轮播图 -->
      ...
      <!-- 不同分类商品 -->
        ...
        <div class="body">
          <GoodsItem v-for="item in sub.goods" :key="item.id" :goods="item" :path="`/product/${item.id}`"></GoodsItem>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  setup(){
    ...
    //获取子目录各个推荐商品 动态路由不会初始化加载，所以要加watch
    const subList=ref([])
    const getSubList=()=>{
